...
exports.getAllMentorGrades = exports.getMentorGrade = exports.submitMentorGrade = void 0;
...
async function verifyMentorAccess(mentorEmail, registrationId) {
    const [rows] = await db_1.default.execute(`SELECT ir.registration_id, ir.mentor_email, s.nim, s.student_name, ir.company_name, ir.semester_code
     FROM internship_registrations ir
     JOIN students s ON ir.nim = s.nim
     WHERE ir.registration_id = ? AND ir.mentor_email = ? AND ir.status = 'approved'`, [registrationId, mentorEmail]);
    return { allowed: rows.length > 0, row: rows[0] };
}
// ─── Submit / Update Nilai ───────────────────────────────────────────────────
// POST /student/mentor/grades/:registration_id
const submitMentorGrade = async (req, res) => {
    ...
    try {
        ...
        const [existing] = await db_1.default.execute('SELECT mentor_score_id FROM mentor_scores WHERE registration_id = ?', [registrationId]);
        if (existing.length > 0) {
            // UPDATE
            await db_1.default.execute(`UPDATE mentor_scores SET
         ...
         WHERE registration_id = ?`, [
            ...
            // INSERT
            await db_1.default.execute(`INSERT INTO mentor_scores
...
exports.submitMentorGrade = submitMentorGrade;
// ─── Get Nilai Satu Mahasiswa ─────────────────────────────────────────────────
// GET /student/mentor/grades/:registration_id
const getMentorGrade = async (req, res) => {
    const mentorEmail = req.user?.email;
    const registrationId = Number(req.params.registration_id);
    if (!mentorEmail) {
        res.status(401).json({ success: false, message: 'Tidak terautentikasi sebagai mentor' });
    ...
    try {
        const [rows] = await db_1.default.execute('SELECT * FROM mentor_scores WHERE registration_id = ?', [registrationId]);
        if (rows.length === 0) {
...
exports.getMentorGrade = getMentorGrade;
// ─── Get Semua Nilai yang Sudah Diinput Mentor ────────────────────────────────
// GET /student/mentor/grades
const getAllMentorGrades = async (req, res) => {
    const mentorEmail = req.user?.email;
    if (!mentorEmail) {
        res.status(401).json({ success: false, message: 'Tidak terautentikasi sebagai mentor' });
        return;
    ...
    try {
        const [rows] = await db_1.default.execute(`SELECT ms.*, s.nim, s.student_name, ir.company_name, ir.semester_code, ir.internship_position
       FROM mentor_scores ms
       JOIN internship_registrations ir ON ms.registration_id = ir.registration_id
       JOIN students s ON ir.nim = s.nim
       WHERE ir.mentor_email = ?
       ORDER BY ms.updated_at DESC`, [mentorEmail]);
...
exports.getAllMentorGrades = getAllMentorGrades;
