...
export default function StudentDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-full flex bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <aside className={`hidden md:flex relative bg-white dark:bg-slate-900 flex-shrink-0 transition-all duration-300 overflow-hidden z-20 border-r border-[#ebebeb] dark:border-slate-700/60 flex-col ${sidebarOpen ? 'w-64' : 'w-[84px]'}`} style={{ boxShadow: '2px 0 12px rgba(0,0,0,0.05)' }}>
      ...
      {/* ── Mobile Sidebar Drawer (hanya mobile) ── */}
      {mobileOpen && (
        <div
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-white dark:bg-slate-900 z-50 flex flex-col md:hidden transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        {/* Isi sidebar sama persis (selalu terbuka / tidak collapsed) */}
          ...
          <div className="flex items-center p-2 rounded-2xl border border-gray-200/60 dark:border-slate-700/60 bg-gray-50 dark:bg-slate-800/40">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400 truncate mt-0.5">{user?.nim}{user?.prodi ? ` • ${user.prodi}` : ''}</p>
              </div>
            </div>
          </div>
        </div>
        <nav className="p-3 flex-1 flex flex-col gap-1 overflow-y-auto mt-1">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`relative flex items-center px-4 py-3 rounded-xl font-medium transition-all duration-150 ${
                  isActive ? 'text-gray-900 dark:text-white bg-gray-100 dark:bg-slate-800/80' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/40'
                }`}
              >
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-[#CC0000] rounded-r-md" />}
                <span className={`flex-shrink-0 flex items-center justify-center w-[28px] ${isActive ? 'text-[#CC0000]' : 'text-gray-400 dark:text-slate-500'} [&>svg]:w-[22px] [&>svg]:h-[22px]`}>{item.icon}</span>
          ...
          <div className="mt-4">
            ...
            <Link href="/pengaturan"
              className={`relative flex items-center px-4 py-3 rounded-xl font-medium transition-all duration-150 ${
                pathname === '/pengaturan' ? 'text-gray-900 dark:text-white bg-gray-100 dark:bg-slate-800/80' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/40'
              }`}
            >
              {pathname === '/pengaturan' && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-[#CC0000] rounded-r-md" />}
              <span className={`flex-shrink-0 flex items-center justify-center w-[28px] ${pathname === '/pengaturan' ? 'text-[#CC0000]' : 'text-gray-400 dark:text-slate-500'} [&>svg]:w-[22px] [&>svg]:h-[22px]`}><SettingsIcon /></span>
              <span className="ml-4 text-[15px]">{t.pengaturan}</span>
            </Link>
          </div>
        </nav>
        <div className="p-3 border-t border-gray-100 dark:border-slate-800">
          <button onClick={handleLogout} disabled={isLoggingOut}
            className="w-full flex items-center px-4 py-3 rounded-xl text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 font-medium transition-all duration-150 disabled:opacity-60"
          >
            <span className="flex-shrink-0 flex items-center justify-center w-[28px] [&>svg]:w-[22px] [&>svg]:h-[22px]"><LogoutIcon /></span>
            <span className="ml-4 text-[15px]">{isLoggingOut ? t.loggingOut : t.keluar}</span>
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        <div className="px-4 pt-4 pb-2 shrink-0 z-40">
          <header className="bg-gradient-to-r from-[#CC0000] to-[#E60000] dark:from-slate-900 dark:to-slate-800 h-14 rounded-[2rem] flex items-center px-4 gap-3 transition-all duration-300 shadow-lg shadow-red-900/15 dark:shadow-black/40 border border-white/20 dark:border-slate-700 relative">
            ...
            <button
              className="md:hidden flex-shrink-0 p-1.5 rounded-xl text-white/80 hover:bg-white/10 active:bg-white/20 transition-colors"
