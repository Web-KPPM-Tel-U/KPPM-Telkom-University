      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-5">
        ...
        {!data ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 p-6 sm:p-10 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 font-semibold text-sm">Belum ada data nilai</p>
            <p className="text-gray-400 dark:text-slate-500 text-xs mt-1">
              Nilai akan tampil di sini setelah pengajuan disetujui.
            </p>
          </div>
        ) : (
          ...
          <div className="md:hidden">
            {!data ? (
              <div className="px-4 py-12 text-center">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto mb-2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14,2 14,8 20,8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg>
                <p className="text-sm font-medium text-gray-400">Belum ada data nilai</p>
                <p className="text-xs text-gray-300 mt-0.5">Nilai akan tampil di sini setelah pengajuan disetujui.</p>
              </div>
            ) : (
              <div className="px-4 py-4 space-y-3">
                {/* Info baris 1: Nama Perusahaan + Posisi */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-0.5">Nama Perusahaan</p>
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 break-words">{data.registration.company_name}</p>
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-0.5">Posisi</p>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-300 break-words max-w-[130px] text-right">{data.registration.internship_position}</p>
                  </div>
                </div>
                {/* Info baris 2: Tanggal pengajuan */}
                <div>
                  <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-0.5">Tanggal Pengajuan</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{fmtDate(data.registration.submitted_at)}</p>
                </div>
                {/* Tombol aksi */}
                <button
                  onClick={() => setView('detail')}
                  className="w-full inline-flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 hover:border-[#CC0000]/50 hover:text-[#CC0000] px-3 py-2 rounded-lg transition-colors"
                >
                ...
                <tr className="bg-[#f4f4f6] dark:bg-gray-700/50 border-b-2 border-[#e5e7eb] dark:border-gray-600">
                  ...
                  <th className="text-right px-6 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <select disabled className="border border-gray-300 dark:border-gray-600 rounded px-1.5 py-0.5 text-xs text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 focus:outline-none cursor-not-allowed"><option>10</option></select>
                ...
                {!data ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-14 text-center">
                      <div className="flex flex-col items-center gap-2">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14,2 14,8 20,8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg>
                        <p className="text-sm font-medium text-gray-400">Belum ada data nilai</p>
                  ...
                  <tr className="transition-colors hover:bg-red-50/30 dark:hover:bg-red-900/10 border-b border-[#f0f0f0] dark:border-gray-700 bg-white dark:bg-gray-800">
                    <td className="px-6 py-4 border-r border-[#f0f0f0] dark:border-gray-700">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{fmtDate(data.registration.submitted_at)}</span>
                    </td>
                    <td className="px-6 py-4 border-r border-[#f0f0f0] dark:border-gray-700">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{data.registration.company_name}</span>
                    </td>
                    <td className="px-6 py-4 border-r border-[#f0f0f0] dark:border-gray-700">
                      <span className="text-sm font-medium text-gray-600 dark:text-gray-300">{data.registration.internship_position}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => setView('detail')} className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 hover:border-[#CC0000]/50 hover:text-[#CC0000] px-3 py-1.5 rounded-lg transition-colors">
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-5">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">Detail Rekap Nilai</h1>
