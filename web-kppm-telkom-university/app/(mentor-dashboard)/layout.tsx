...
import { getToken, getUser, logout, removeToken, getMentorDashboard } from '@/lib/api';
...
export default function MentorDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const getInitials = (nameOrEmail?: string) => {
    if (!nameOrEmail) return 'MT';
    if (nameOrEmail.includes('@')) return nameOrEmail.substring(0, 2).toUpperCase();
    return nameOrEmail.split(' ').map((n) => n[0]).slice(0, 2).join('').toUpperCase();
  };
          ...
          <div className="relative flex items-center p-2 rounded-2xl border border-gray-200/60 dark:border-slate-700/60 bg-gray-50 dark:bg-slate-800/40 transition-all duration-300 overflow-hidden justify-start">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl bg-[#CC0000]/10 dark:bg-red-500/20 text-[#CC0000] dark:text-red-400 flex items-center justify-center font-bold text-sm flex-shrink-0" style={{ boxShadow: 'inset 0 0 0 1px rgba(204,0,0,0.1)' }}>
                {mentor ? getInitials(mentor.name || mentor.email) : '?'}
              </div>
              <div className={`transition-all duration-300 ease-in-out overflow-hidden ${sidebarOpen ? 'w-[150px] opacity-100' : 'w-0 opacity-0'}`}>
                <div className="w-[150px] flex flex-col justify-center">
                  <p className="text-sm font-bold text-gray-900 dark:text-white truncate whitespace-nowrap">{mentor?.name || mentor?.email || 'Mentor'}</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400 truncate whitespace-nowrap mt-0.5">Portal Mentor</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={`relative flex items-center px-4 py-3 rounded-xl font-medium transition-all duration-150 ${isActive
      ...
      >
        ...
        <div className="p-3 border-b border-gray-100 dark:border-slate-800">
          <div className="flex items-center p-2 rounded-2xl border border-gray-200/60 dark:border-slate-700/60 bg-gray-50 dark:bg-slate-800/40">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl bg-[#CC0000]/10 text-[#CC0000] flex items-center justify-center font-bold text-sm flex-shrink-0">{mentor ? getInitials(mentor.name || mentor.email) : '?'}</div>
              <div className="min-w-0">
                <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{mentor?.name || mentor?.email || 'Mentor'}</p>
                <p className="text-xs text-gray-500 truncate mt-0.5">Portal Mentor</p>
              </div>
            </div>
          </div>
        </div>
