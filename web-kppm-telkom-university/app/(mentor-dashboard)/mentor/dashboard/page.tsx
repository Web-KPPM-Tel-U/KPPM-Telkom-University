...
function MenteeCard({ mentee, idx }: { mentee: MentorMentee; idx: number }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      {/* Card Header */}
      <div className="p-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-bold text-gray-900 dark:text-slate-100 text-sm">{mentee.student.name}</p>
      ...
      <button
        ...
        className="w-full px-5 py-2.5 text-xs font-semibold text-[#CC0000] dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border-t border-gray-100 dark:border-slate-800 transition-colors flex items-center justify-center gap-1.5"
      >
        {open ? '▲ Sembunyikan Detail' : '▼ Lihat Detail'}
      </button>

      {open && (
        <div className="px-5 pb-5 pt-3 border-t border-gray-50 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Email Mahasiswa</p>
              <p className="flex items-center gap-1.5 text-xs text-gray-700 dark:text-slate-300 truncate" title={mentee.student.email}>
                <span className="flex-shrink-0"><MailIcon /></span>
                <span className="truncate">{mentee.student.email}</span>
              </p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">WhatsApp</p>
              <p className="flex items-center gap-1.5 text-xs text-gray-700 dark:text-slate-300 truncate">
                <span className="flex-shrink-0"><PhoneIcon /></span>
                <span className="truncate">+{mentee.student.whatsapp}</span>
              </p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Semester</p>
              <p className="text-xs text-gray-700 dark:text-slate-300 truncate">{mentee.semester_code}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Dosen Pembimbing</p>
              <p className="text-xs text-gray-700 dark:text-slate-300 truncate" title={mentee.pembimbing_akademik || '-'}>{mentee.pembimbing_akademik || '-'}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Disetujui Pada</p>
              <p className="text-xs text-gray-700 dark:text-slate-300 truncate">{formatDate(mentee.approved_at)}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Perusahaan</p>
              <p className="text-xs text-gray-700 dark:text-slate-300 truncate" title={mentee.company_name}>{mentee.company_name}</p>
            </div>
          </div>
        </div>
      )}
    ...
    return (
      <div className="p-6 max-w-lg mx-auto mt-8">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6 text-center">
          ...
          <p className="text-red-700 dark:text-red-400 font-semibold mb-1">Koneksi Bermasalah</p>
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-5">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm p-5">
        <h2 className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-4">Informasi Anda</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Email</p>
            <p className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-slate-300 font-medium">
              <MailIcon /> {mentor?.email}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">Jabatan</p>
            <p className="flex items-center gap-1.5 text-sm text-gray-700 dark:text-slate-300 font-medium">
      ...
      <div>
        ...
        {mentees.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 p-6 sm:p-10 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 font-semibold text-sm">Belum ada mahasiswa yang disetujui</p>
